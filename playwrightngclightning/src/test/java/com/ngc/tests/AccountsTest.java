...
//	@Test
	public void CollectionIncidentDetails() throws InterruptedException {
		...
		Thread.sleep(3000);
		Locator Decision =  page.getByRole(AriaRole.ROW, new Page.GetByRoleOptions().setName("Trigger Date Assigned To Decision Method Status Risk Contact Follow Up Resolved % Offsite % Late Locks Risk Rank")).getByText("Decision");
		String generatedDecisionText = Decision.textContent();
		assertTrue(true,"Decision");
		
		Locator Method =  page.getByRole(AriaRole.ROW, new Page.GetByRoleOptions().setName("Trigger Date Assigned To Decision Method Status Risk Contact Follow Up Resolved % Offsite % Late Locks Risk Rank")).getByText("Method");
		String generatedMethodText = Method.textContent();
		assertTrue(true,"Method");
		
		Locator Status =    page.getByRole(AriaRole.ROW, new Page.GetByRoleOptions().setName("Trigger Date Assigned To Decision Method Status Risk Contact Follow Up Resolved % Offsite % Late Locks Risk Rank")).getByText("Status");
		String generatedStatusText = Status.textContent();
		assertTrue(true,"Status");
		
		Locator Comments_for_Selected_Incident =   page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Comments for Selected Incident"));
		...
		Locator Locks_Reasons_for_Selected_Incident =    page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Locks/Reasons for Selected Incident"));
		...
		Locator History_for_Selected_Incident =    page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("History for Selected Incident"));
	...
	public void AccountAddress() throws InterruptedException {
		...
		Locator AccountAddress =  page.getByRole(AriaRole.BUTTON, new Page.GetByRoleOptions().setName("Address Information"));
